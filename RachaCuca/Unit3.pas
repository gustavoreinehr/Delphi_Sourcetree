...
begin
  case ComboBoxCamiseta2.ItemIndex of
   ...
   PanelComboBox2.Repaint;   //forçar repintura imediata
...
var
  i, posEvaristo, posInglesa, posAlema: Integer;
...
begin
  ...
  for i := 1 to 5 do
  begin
    if SameText(Trim(nomes[i].Text), 'Evaristo') then posEvaristo := i;
    if SameText(Trim(cervejas[i].Text), 'inglesa') then posInglesa := i;
    if SameText(Trim(cervejas[i].Text), 'alemã') then posAlema := i;
  end;

  if (posEvaristo in [4, 5]) or (posInglesa in [1, 2]) then
  begin
    Dica1.Font.Color := clRed;
    Dica1.Font.Style := Dica1.Font.Style + [fsStrikeOut];
  end
  else if posAlema <> -1 then
  begin
    if (posEvaristo <> -1) and (posInglesa <> -1) then
    begin
      if (posEvaristo < posAlema) and (posAlema < posInglesa) then
      begin
    ...
    else if posEvaristo <> -1 then
    begin
      if posEvaristo >= posAlema then
      begin
        Dica1.Font.Color := clRed;
        Dica1.Font.Style := Dica1.Font.Style + [fsStrikeOut];
      end
      ...
      begin
        var temEspacoDireita := False;
        for i := posAlema + 1 to 5 do
        begin
          if (Trim(cervejas[i].Text) = '') or SameText(Trim(cervejas[i].Text), 'inglesa') then
          begin
            temEspacoDireita := True;
            Break;
          end;
        end;
        if not temEspacoDireita then
        begin
          Dica1.Font.Color := clRed;
          Dica1.Font.Style := Dica1.Font.Style + [fsStrikeOut];
        end
    ...
    else if posInglesa <> -1 then
    begin
      if posInglesa <= posAlema then
      begin
        Dica1.Font.Color := clRed;
        Dica1.Font.Style := Dica1.Font.Style + [fsStrikeOut];
      end
      ...
      begin
        var temEspacoEsquerda := False;
        for i := 1 to posAlema - 1 do
        begin
          if (Trim(nomes[i].Text) = '') or SameText(Trim(nomes[i].Text), 'Evaristo') then
          begin
            temEspacoEsquerda := True;
            Break;
          end;
        end;
        if not temEspacoEsquerda then
        begin
          Dica1.Font.Color := clRed;
          Dica1.Font.Style := Dica1.Font.Style + [fsStrikeOut];
        end
    ...
    begin
      if (posAlema = 1) or (posAlema = 5) then
      begin
        Dica1.Font.Color := clRed;
        Dica1.Font.Style := Dica1.Font.Style + [fsStrikeOut];
      end
      ...
      begin
        var temNomeEsquerdaInvalido := True;
        for i := 1 to posAlema - 1 do
        begin
          if (Trim(nomes[i].Text) = '') or SameText(Trim(nomes[i].Text), 'Evaristo') then //verifica vazio ou evaristo
          begin
            temNomeEsquerdaInvalido := False;
            Break;
          end;
        end;

        var temCervejaDireitaInvalida := True;      // declara somente agora para evitar uso desnecessario
        for i := posAlema + 1 to 5 do
        begin
          if (Trim(cervejas[i].Text) = '') or SameText(Trim(cervejas[i].Text), 'inglesa') then
          begin
            temCervejaDireitaInvalida := False;
            Break;
          end;
        end;

        if temNomeEsquerdaInvalido or temCervejaDireitaInvalida then
        begin
          Dica1.Font.Color := clRed;
          Dica1.Font.Style := Dica1.Font.Style + [fsStrikeOut];
        end
  ...
  begin
    if (posEvaristo <> -1) and (posInglesa <> -1) then
    begin
      if (posEvaristo >= posInglesa) then
      begin
        Dica1.Font.Color := clRed;
        Dica1.Font.Style := Dica1.Font.Style + [fsStrikeOut];
      end
      ...
      begin
        var temEspacoEntre := False;
        for i := posEvaristo + 1 to posInglesa - 1 do      //verifica se tem uma posicao vazia entre evaristo
        begin                                             // e cerveja inglesa
        ...
        if not temEspacoEntre then
        begin
          Dica1.Font.Color := clRed;
          Dica1.Font.Style := Dica1.Font.Style + [fsStrikeOut];
        end
        else          // para caso nao entre na condicao tem espaço entre e nao tem espaço entre
        begin
          Dica1.Font.Color := clBlack;
          Dica1.Font.Style := Dica1.Font.Style - [fsStrikeOut];
        end;
    ...
    else          // verifica posicao nula dos nome e cerveja
    begin
      Dica1.Font.Color := clBlack;
      Dica1.Font.Style := Dica1.Font.Style - [fsStrikeOut];
    end;
...
var
  i, posBasquete, posFutebol, posSurfe: Integer;
...
begin
  ...
  for i := 1 to 5 do
  begin
    if SameText(Trim(esportes[i].Text), 'basquete') then posBasquete := i;
    if SameText(Trim(esportes[i].Text), 'futebol') then posFutebol := i;
    if SameText(Trim(esportes[i].Text), 'surfe') then posSurfe := i;
  end;

  if (posFutebol in [4, 5]) or (posSurfe in [1, 2]) then
  begin
    Dica11.Font.Color := clRed;
    Dica11.Font.Style := Dica11.Font.Style + [fsStrikeOut];
  end
  else if posBasquete <> -1 then
  begin
    if (posFutebol <> -1) and (posSurfe <> -1) then
    begin
      if (posFutebol < posBasquete) and (posBasquete < posSurfe) then
      begin
    ...
    else if posFutebol <> -1 then
    begin
      if posFutebol >= posBasquete then
      begin
        Dica11.Font.Color := clRed;
        Dica11.Font.Style := Dica11.Font.Style + [fsStrikeOut];
      end
      ...
      begin
        ...
        begin
          if (Trim(esportes[i].Text) = '') or SameText(Trim(esportes[i].Text), 'surfe') then
          begin
            temEspacoDireita := True;
            Break;
          end;
    ...
    else if posSurfe <> -1 then
    begin
      if posSurfe <= posBasquete then
      begin
        Dica11.Font.Color := clRed;
        Dica11.Font.Style := Dica11.Font.Style + [fsStrikeOut];
      end
      ...
      begin
        ...
        begin
          if (Trim(esportes[i].Text) = '') or SameText(Trim(esportes[i].Text), 'futebol') then
          begin
            temEspacoEsquerda := True;
            Break;
          end;
    ...
    begin
      if (posBasquete = 1) or (posBasquete = 5) then
      begin
        Dica11.Font.Color := clRed;
        Dica11.Font.Style := Dica11.Font.Style + [fsStrikeOut];
      end
      ...
      begin
        ...
        if not temEspacoEsquerda or not temEspacoDireita then
        begin
          Dica11.Font.Color := clRed;
          Dica11.Font.Style := Dica11.Font.Style + [fsStrikeOut];
        end
  ...
  begin
    ...
    begin
      ...
      begin
        var temEspacoEntre := False;
        for i := posFutebol + 1 to posSurfe - 1 do
        begin
...
begin
  ...
  else if (posFutebol = 1) and futebolPreenchido then
  begin
    Dica7.Font.Style := Dica7.Font.Style + [fsStrikeOut];
    Dica7.Font.Color := clRed;
  end
  else if (posFutebol > 1) and (Trim(cervejas[posFutebol - 1].Text) <> '') and (LowerCase(Trim(cervejas[posFutebol - 1].Text)) <> 'irlandesa') and futebolPreenchido then
  begin
    Dica7.Font.Style := Dica7.Font.Style + [fsStrikeOut];
    Dica7.Font.Color := clRed;
  end
...
procedure TForm3.VerificarComboBox;
var
  i: Integer;
  nomes: array[1..5] of TComboBox;
  camisetas: array[1..5] of TComboBox;
  hamburgueres: array[1..5] of TComboBox;
  cervejas: array[1..5] of TComboBox;
  idades: array[1..5] of TComboBox;
  esportes: array[1..5] of TComboBox;
  valoresNomes: array[1..5] of string;
  valoresCamisetas: array[1..5] of string;
  valoresHamburgueres: array[1..5] of string;
  valoresCervejas: array[1..5] of string;
  valoresIdades: array[1..5] of string;
  valoresEsportes: array[1..5] of string;
  tudoCorreto: Boolean;
  temNulo: Boolean;
begin
  nomes[1] := ComboBoxNome;
  ...
  hamburgueres[1] := ComboBoxHamburguer;
  hamburgueres[2] := ComboBoxHamburguer2;
  hamburgueres[3] := ComboBoxHamburguer3;
  hamburgueres[4] := ComboBoxHamburguer4;
  hamburgueres[5] := ComboBoxHamburguer5;
  ...
  valoresNomes[1] := 'Daniel';
  valoresNomes[2] := 'Evaristo';
  valoresNomes[3] := 'Antônio';
  valoresNomes[4] := 'Vinicius';
  valoresNomes[5] := 'Paulo';

  valoresCamisetas[1] := 'branca';
  valoresCamisetas[2] := 'amarela';
  valoresCamisetas[3] := 'azul';
  valoresCamisetas[4] := 'vermelha';
  valoresCamisetas[5] := 'verde';

  valoresHamburgueres[1] := 'sem alface';
  valoresHamburgueres[2] := 'bem passado';
  valoresHamburgueres[3] := 'bacon extra';
  valoresHamburgueres[4] := 'onion rings';
  valoresHamburgueres[5] := 'sem cebola';

  valoresCervejas[1] := 'irlandesa';
  valoresCervejas[2] := 'belga';
  valoresCervejas[3] := 'alemã';
  valoresCervejas[4] := 'inglesa';
  valoresCervejas[5] := 'holandesa';

  valoresIdades[1] := '34 anos';
  valoresIdades[2] := '28 anos';
  valoresIdades[3] := '26 anos';
  valoresIdades[4] := '30 anos';
  valoresIdades[5] := '32 anos';

  valoresEsportes[1] := 'vôlei';
  valoresEsportes[2] := 'futebol';
  valoresEsportes[3] := 'corrida';
  valoresEsportes[4] := 'basquete';
  valoresEsportes[5] := 'surfe';
  ...
  begin
    ...
    if Trim(camisetas[i].Text) = '' then
      temNulo := True;
    if Trim(hamburgueres[i].Text) = '' then
      temNulo := True;
    if Trim(cervejas[i].Text) = '' then
      temNulo := True;
  ...
  begin
    for i := 1 to 5 do
    begin
      if Trim(nomes[i].Text) <> valoresNomes[i] then
        tudoCorreto := False;
      if Trim(camisetas[i].Text) <> valoresCamisetas[i] then
        tudoCorreto := False;
      if Trim(hamburgueres[i].Text) <> valoresHamburgueres[i] then
        tudoCorreto := False;
      if Trim(cervejas[i].Text) <> valoresCervejas[i] then
        tudoCorreto := False;
      if Trim(idades[i].Text) <> valoresIdades[i] then
        tudoCorreto := False;
      if Trim(esportes[i].Text) <> valoresEsportes[i] then
        tudoCorreto := False;
    end;

    if tudoCorreto then
      ShowMessage('Parabéns, você completou o teste!');
  end;
end;

procedure TForm3.VerificarDicaAmareloEntreHamburguerSemAlfaceEVermelho;
var
  i, posAmarela, posSemAlface, posVermelha: Integer;
  camisetas: array[1..5] of TComboBox;
...
begin
  ...
  for i := 1 to 5 do
  begin
    if SameText(Trim(camisetas[i].Text), 'amarela') then posAmarela := i;
    if SameText(Trim(pedidos[i].Text), 'sem alface') then posSemAlface := i;
    if SameText(Trim(camisetas[i].Text), 'vermelha') then posVermelha := i;
  end;

  if (posSemAlface in [4, 5]) or (posVermelha in [1, 2]) then
  begin
  ...
  if (posAmarela = 1) or (posAmarela = 5) then
  begin
    Dica10.Font.Color := clRed;
    Dica10.Font.Style := Dica10.Font.Style + [fsStrikeOut];
    Exit;
...
begin
  ...
  if (Trim(idades[1].Text) <> '') and (LowerCase(Trim(idades[1].Text)) = '28 anos') then
  begin
  ...
  if (Trim(esportes[5].Text) <> '') and (LowerCase(Trim(esportes[5].Text)) = 'corrida') then
  begin
    Dica19.Font.Style := Dica19.Font.Style + [fsStrikeOut];
    Dica19.Font.Color := clRed;
    Exit;
...
var
  ...
  danielDireita: Boolean;
  danielQuintaPosicao: Boolean;
  idadePreenchidaDireita: Boolean;
  idade28EsquerdaDaniel: Boolean;
  posicaoDaniel: Integer;
begin
  nomes[1] := ComboBoxNome;
  nomes[2] := ComboBoxNome2;
  nomes[3] := ComboBoxNome3;
  ...
  danielDireita := False;
  danielQuintaPosicao := False;
  idadePreenchidaDireita := False;
  idade28EsquerdaDaniel := False;
  posicaoDaniel := 0;
  ...
  for i := 2 to 5 do
  begin
    if (Trim(nomes[i].Text) <> '') and (LowerCase(Trim(nomes[i].Text)) = 'daniel') then
    begin
      danielDireita := True;
      posicaoDaniel := i;
      if i = 5 then
        danielQuintaPosicao := True;
    end;
  end;
  ...
  begin
    if Trim(idades[i].Text) <> '' then
    begin
      idadePreenchidaDireita := True;
      Break;
    end;
  end;

  if (posicaoDaniel > 0) then
  begin
    for i := 1 to posicaoDaniel - 1 do
    begin
      if (Trim(idades[i].Text) <> '') and (LowerCase(Trim(idades[i].Text)) = '28 anos') then
      begin
        idade28EsquerdaDaniel := True;
        Break;
      end;
    end;
  end;
  ...
  else if (algumErro) or (danielQuintaPosicao) or (danielDireita and idadePreenchidaDireita) or (idade28EsquerdaDaniel) then
  begin
    Dica3.Font.Style := Dica3.Font.Style + [fsStrikeOut];
...
var
  i, posEvaristo, posAzul: Integer;
  nomes: array[1..5] of TComboBox;
  camisas: array[1..5] of TComboBox;
  adjacenteAzul, ladosVazios, avaliarLados, evaristoDistante, mesmaPosicao: Boolean;
begin
  nomes[1] := ComboBoxNome;
  nomes[2] := ComboBoxNome2;
  nomes[3] := ComboBoxNome3;
  ...
  if (posEvaristo = posAzul) and (posEvaristo <> -1) then
  begin
    mesmaPosicao := True;
    Dica18.Font.Style := Dica18.Font.Style + [fsStrikeOut];
  ...
  if (posAzul > 1) and (Trim(nomes[posAzul - 1].Text) = '') and
     (posAzul < 5) and (Trim(nomes[posAzul + 1].Text) = '') then
    ladosVazios := True;

  if (posAzul > 1) and (LowerCase(Trim(nomes[posAzul - 1].Text)) = 'evaristo') then
    adjacenteAzul := True;

  if (posAzul < 5) and (LowerCase(Trim(nomes[posAzul + 1].Text)) = 'evaristo') then
    adjacenteAzul := True;

  if (posAzul > 1) and (posAzul < 5) and (Trim(nomes[posAzul - 1].Text) <> '') and
     (LowerCase(Trim(nomes[posAzul - 1].Text)) <> 'evaristo') and
     (Trim(nomes[posAzul + 1].Text) <> '') and
     (LowerCase(Trim(nomes[posAzul + 1].Text)) <> 'evaristo') then
    avaliarLados := True;

  if (posEvaristo <> -1) and (Abs(posEvaristo - posAzul) > 1) then
    evaristoDistante := True;

  if adjacenteAzul then
  begin
    Dica18.Font.Style := Dica18.Font.Style + [fsStrikeOut];
    Dica18.Font.Color := clGreen;
  end
  else if ladosVazios then
  begin
    Dica18.Font.Style := Dica18.Font.Style - [fsStrikeOut];
    Dica18.Font.Color := clBlack;
  end
  else if avaliarLados or evaristoDistante or
          ((posAzul = 1) and (Trim(nomes[2].Text) <> '') and
           (LowerCase(Trim(nomes[2].Text)) <> 'evaristo')) or
          ((posAzul = 5) and (Trim(nomes[4].Text) <> '') and
           (LowerCase(Trim(nomes[4].Text)) <> 'evaristo')) then
  begin
    Dica18.Font.Style := Dica18.Font.Style + [fsStrikeOut];
    Dica18.Font.Color := clRed;
  end;
...
begin
  ...
  for i := 1 to 5 do
  begin
    if (Trim(idades[i].Text) <> '') and (LowerCase(Trim(idades[i].Text)) = '28 anos') and
       (Trim(camisetas[i].Text) <> '') and (LowerCase(Trim(camisetas[i].Text)) = 'branca') then
    begin
      mesmaPosicao := True;
      Break;
    end;
...
begin
  ...
  if (Trim(cervejas[posMaisVelho].Text) <> '') and SameText(Trim(cervejas[posMaisVelho].Text), 'belga') then
  begin
    mesmaPosicaoBelga := True;
  ...
  if (posMaisVelho = 1) and ladoDireitoPreenchido and (ladoDireitoCorreto = False) then
  begin
    Dica13.Font.Color := clRed;
    if not (fsStrikeOut in Dica13.Font.Style) then
      Dica13.Font.Style := Dica13.Font.Style + [fsStrikeOut];
  ...
  if (posMaisVelho = 5) and ladoEsquerdoPreenchido and (ladoEsquerdoCorreto = False) then
  begin
    Dica13.Font.Color := clRed;
    if not (fsStrikeOut in Dica13.Font.Style) then
      Dica13.Font.Style := Dica13.Font.Style + [fsStrikeOut];
  ...
  if (posMaisVelho > 1) and (posMaisVelho < 5) and ladoEsquerdoPreenchido and ladoDireitoPreenchido and (ladoEsquerdoCorreto = False) and (ladoDireitoCorreto = False) then
  begin
    Dica13.Font.Color := clRed;
    if not (fsStrikeOut in Dica13.Font.Style) then
      Dica13.Font.Style := Dica13.Font.Style + [fsStrikeOut];
...
var
  i, posInglesa, posHolandesa: Integer;
  cervejas: array[1..5] of TComboBox;
  encontrou: Boolean;
...
begin
  ...
  begin
    if LowerCase(Trim(cervejas[i].Text)) = 'inglesa' then
      posInglesa := i;
    if LowerCase(Trim(cervejas[i].Text)) = 'holandesa' then
      posHolandesa := i;
  end;

  if (posInglesa <> -1) then
  begin
    if posInglesa = 5 then
    begin
  ...
  else if (posHolandesa <> -1) then
  begin
    if posHolandesa = 1 then
    begin
      Dica12.Font.Color := clRed;
      if not (fsStrikeOut in Dica12.Font.Style) then
        Dica12.Font.Style := Dica12.Font.Style + [fsStrikeOut];
    ...
    begin
      if Trim(cervejas[posHolandesa - 1].Text) <> '' then
      begin
        if LowerCase(Trim(cervejas[posHolandesa - 1].Text)) <> 'inglesa' then
        begin
          Dica12.Font.Color := clRed;
          if not (fsStrikeOut in Dica12.Font.Style) then
            Dica12.Font.Style := Dica12.Font.Style + [fsStrikeOut];
...
begin
  ...
  for i := 1 to 5 do
  begin
    if LowerCase(Trim(idades[i].Text)) = '26 anos' then
      pos26 := i;

    if LowerCase(Trim(idades[i].Text)) = '30 anos' then
      pos30 := i;

    if (i <= 4) and (Trim(idades[i].Text) <> '') and (Trim(idades[i+1].Text) <> '') then
    begin
  ...
  else if (pos30 = 1) and (Trim(idades[1].Text) <> '') then
  begin
    Dica8.Font.Style := Dica8.Font.Style + [fsStrikeOut];
    Dica8.Font.Color := clRed;
  end
  else if (pos26 > pos30) and (pos26 <> -1) and (pos30 <> -1) then
  begin
    Dica8.Font.Style := Dica8.Font.Style + [fsStrikeOut];
    Dica8.Font.Color := clRed;
  end
  else if (pos30 > 1) and (Trim(idades[pos30 - 1].Text) <> '') and (LowerCase(Trim(idades[pos30 - 1].Text)) <> '26 anos') then
  begin
    Dica8.Font.Style := Dica8.Font.Style + [fsStrikeOut];
    Dica8.Font.Color := clRed;
  end
  else if algumErro and idadeDireitaPreenchida and (pos26 <> -1) and (pos30 <> -1) then
  begin
    Dica8.Font.Style := Dica8.Font.Style + [fsStrikeOut];
    Dica8.Font.Color := clRed;
  end
...
begin
  ...
  begin
    mesmaPosicaoVerde := (Trim(camisetas[posOnionRings].Text) <> '') and (LowerCase(Trim(camisetas[posOnionRings].Text)) = 'verde');
    if mesmaPosicaoVerde then
      riscarVermelho := True;

    if posOnionRings = 1 then
    begin
      direitaValida := Trim(pedidos[posOnionRings + 1].Text) <> '';
      if direitaValida then
      begin
        if LowerCase(Trim(pedidos[posOnionRings + 1].Text)) = 'onion rings' then
          riscarVerde := True
        else
          riscarVermelho := True;
      end;
      ...
      begin
        ...
        if direitaVerde then
          riscarVerde := True
        else
          riscarVermelho := True;
      end;
    end
    else if posOnionRings = 5 then
    begin
      esquerdaValida := Trim(pedidos[posOnionRings - 1].Text) <> '';
    ...
    begin
      ...
      if (not (esquerdaVerde or direitaVerde)) and (esquerdaValida and direitaValida) and
         (esquerdaOutraCor and direitaOutraCor) then
        riscarVermelho := True
      else if not mesmaPosicaoVerde then
        riscarVermelho := False;
    end;
  end;
...
var
  i, posPaulo, posAzul: Integer;
  nomes: array[1..5] of TComboBox;
  camisas: array[1..5] of TComboBox;
  esquerdaOk, direitaCompleta, temPauloDireita: Boolean;
begin
  nomes[1] := ComboBoxNome;
  nomes[2] := ComboBoxNome2;
  nomes[3] := ComboBoxNome3;
  ...
  for i := 1 to 5 do
  begin
    if LowerCase(Trim(nomes[i].Text)) = 'paulo' then posPaulo := i;
    if LowerCase(Trim(camisas[i].Text)) = 'azul' then posAzul := i;
  end;
  ...
  if posAzul = -1 then Exit;

  if posPaulo = posAzul then
  begin
    Dica14.Font.Color := clRed;
    Dica14.Font.Style := Dica14.Font.Style + [fsStrikeOut];
    Exit;
  ...
  if posAzul > 1 then
  begin
    for i := 1 to posAzul - 1 do
    begin
      if (nomes[i].Text = '') or (LowerCase(Trim(nomes[i].Text)) = 'paulo') then
      begin
        esquerdaOk := False;
        Break;
      end;
  ...
  temPauloDireita := False;
  direitaCompleta := True;

  if posAzul < 5 then
  begin
    for i := posAzul + 1 to 5 do
    begin
      if nomes[i].Text = '' then
        direitaCompleta := False
      else if LowerCase(Trim(nomes[i].Text)) = 'paulo' then
        temPauloDireita := True;
    end;
  end
  else
    direitaCompleta := False;

  if esquerdaOk and direitaCompleta and not temPauloDireita then
  begin
    Dica14.Font.Color := clRed;
    Dica14.Font.Style := Dica14.Font.Style + [fsStrikeOut];
    Exit;
  ...
  if (posPaulo = -1) and direitaCompleta then
  begin
    Dica14.Font.Color := clRed;
    Dica14.Font.Style := Dica14.Font.Style + [fsStrikeOut];
    Exit;
  ...
  if (posPaulo <> -1) and (posPaulo > posAzul) then
  begin
    Dica14.Font.Color := clGreen;
    Dica14.Font.Style := Dica14.Font.Style + [fsStrikeOut];
  end
  else if (posPaulo <> -1) and (posPaulo < posAzul) then
  begin
    Dica14.Font.Color := clRed;
    Dica14.Font.Style := Dica14.Font.Style + [fsStrikeOut];
  end;
...
begin
  ...
  begin
    ...
    begin
      posVinicius := i;
      viniciusVazio := (Trim(nomes[i].Text) = '');     // verifica se está vazio
      Break;
    end;
  end;

  if (posVinicius = -1) or viniciusVazio then   // verifica se está vazio ou se o campo onde vinicius estava
  begin                                         // está vazio
    Dica16.Font.Color := clBlack;
    Dica16.Font.Style := Dica16.Font.Style - [fsStrikeOut];
    Exit;
  end
  else if posVinicius = 5 then
  begin
    Dica16.Font.Style := Dica16.Font.Style + [fsStrikeOut];
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if Assigned(comboAtual) and (comboAtual.ItemIndex > 0) and        // assigned serve para verificar
             (LowerCase(Trim(comboAtual.Text)) = '30 anos') then            // se a combobox esta criada
          begin                                                              // na memória
            temTrintaEmOutraPosicao := True;
            Break;
          end;
